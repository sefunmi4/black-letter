...
import { Guild, Quest, GuildMembership, User } from '@/api/entities';
...
import QuestCard from '../components/dashboard/QuestCard'; // Assuming QuestCard can be reused
...
export default function GuildDetail() {
    ...
    const loadGuildData = useCallback(async () => {
        ...
        try {
            ...
            if (guildData.length > 0) {
                ...
                // Fetch quests associated with this guild
                // If it's a party, quests are linked via party_id and are private
                // If it's a regular guild, quests are linked via guild_id and are public
                const questFilter = currentGuild.is_party
                    ? { party_id: guildId }
                    : { guild_id: guildId, is_public: true }; // Only fetch public quests for regular guilds
                ...
                if (user) {
                    const isUserMember = membersData.some(m => m.user_email === user.email);
    ...
    useEffect(() => {
        loadGuildData();
    }, [loadGuildData]); // Depend on loadGuildData callback
        ...
        try {
            if (isMember) {
                ...
                if (membership.length > 0) {
                    ...
                    await Guild.update(guild.id, { member_count: (guild.member_count || 1) - 1 });
                    setIsMember(false);
                }
            } else {
                // Join guild
                await GuildMembership.create({ guild_id: guildId, user_email: currentUser.email });
                await Guild.update(guild.id, { member_count: (guild.member_count || 0) + 1 });
                setIsMember(true);
            }
            await loadGuildData(); // Re-fetch data to reflect changes
        } catch (error) {
            console.error("Error joining/leaving guild:", error);
        } finally {
            setIsLoading(false);
        }
    };
                                ...
                                {quests.length > 0 ? (
                                    quests.map(quest => (
                                        <QuestCard
                                            ...
                                            onQuestUpdate={loadGuildData} // Pass loadGuildData to refresh quests
                                        />
                                    ))
                                ) : (
