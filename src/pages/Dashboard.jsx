...
import { Quest, Guild, User, QuestLike, QuestComment } from "@/api/entities"; // Added QuestComment
...
import FeaturedQuests from "../components/dashboard/FeaturedQuests";
...
export default function Dashboard() {
  ...
  const [userQuests, setUserQuests] = useState([]); // Add state for user's quests
  ...
  const loadData = useCallback(async () => {
    setIsLoading(true);
    try {
      // Fetch user data, quests, and guilds concurrently
      const [questData, guildData, me] = await Promise.all([
        Quest.filter({ is_public: true }, "-created_date", 100), // Only fetch public quests for the main feed
        Guild.list("-created_date", 10),
        User.me().catch(() => null) // Handle user not logged in gracefully
      ]);

      // Get all unique creator emails from quests
      const creatorEmails = [...new Set(questData.map(quest => quest.created_by))];
      
      // Fetch user data for all creators
      let creators = [];
      try {
        const allUsers = await User.list();
        creators = allUsers.filter(user => creatorEmails.includes(user.email));
      } catch (error) {
      ...
      // Filter out archived quests from the main feed
      const activeQuests = questData.filter(quest => !quest.is_archived);

      // Calculate actual counts for each quest from likes and comments
      const questsWithCounts = await Promise.all(
        activeQuests.map(async (quest) => {
          // Fetch likes and comments for each quest
          ...
          return {
            ...quest,
            creator_user: creatorMap.get(quest.created_by), // Add creator user data
      ...
      setQuests(questsWithCounts); // Use the quests with calculated counts
      setGuilds(guildData);
      setCurrentUser(me);

      // Fetch user's own quests for team applications (including archived ones for workspace)
      if (me) {
        const userQuestData = await Quest.filter({ created_by: me.email }, "-created_date");
        setUserQuests(userQuestData);
        
        // Fetch user's likes only if logged in
        ...
        // Clear likes and user quests if user is not logged in
      ...
      <div className="w-full max-w-6xl mx-auto px-3 sm:px-4 md:px-6 py-4 md:py-6 space-y-4 sm:space-y-6 md:space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-8 items-start">
          ...
          <div className="lg:col-span-3 space-y-3 sm:space-y-4 md:space-y-6 w-full">
            ...
            {/* Open Opportunities */}
            <FeaturedQuests quests={quests} isLoading={isLoading} currentUser={currentUser} />
